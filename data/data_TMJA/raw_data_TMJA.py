...
data_densite= pd.read_csv("Projet/data/data_TMJA/tmja-2019.csv", sep=";")
...
data[data["depPrD"]==28][["depPrD", "anneeMesureTrafic", "TMJA"]]
data[data["depPrD"]==92]["TMJA"].mean()
data[data["depPrD"]==92]["TMJA"].sum()

# Ajout d'une colonne departement et d'une colonne region
data["depPrD"]=data["depPrD"].astype(str)
...
data["num_dep"]=data["depPrD"]
data["departement"]=data["depPrD"].map(departements)
data["region"]=data["depPrD"].map(regions)

data.info()
data.head()

# Creer un fichier csv pour les données nettoyées et enrichies des denpartements et regions

#dep:
data_netoye = data[["num_dep", "departement", "anneeMesureTrafic", "TMJA"]]

data_netoye.to_csv("Projet/data/data_TMJA/tmja-departement.csv", index=False)

#reg:
data_netoye=data[["num_dep", "region", "anneeMesureTrafic", "TMJA"]]
data_netoye["region"].unique()
data_netoye.to_csv("Projet/data/data_TMJA/tmja-region.csv", index=False)


#Cumul des TMJA par region et par année. Enregistrement des données dans un fichier csv
data_TMJA_sum_reg=data.groupby(["region"])["TMJA"].sum().sort_values(ascending=False)
data_TMJA_mean_reg=data.groupby(["region"])["TMJA"].mean().sort_values(ascending=False)

data_TMJA_sum_reg.to_csv("Projet/data/data_TMJA/tmja-region-sum.csv", header=True)
data_TMJA_mean_reg.to_csv("Projet/data/data_TMJA/tmja-region-mean.csv", header=True)

#Cumul des TMJA par departement et par année. Enregistrement des données dans un fichier csv
data_TMJA_sum_dep=data.groupby(["departement"])["TMJA"].sum().sort_values(ascending=False)
data_TMJA_mean_dep=data.groupby(["departement"])["TMJA"].mean().sort_values(ascending=False)

data_TMJA_sum_dep.to_csv("Projet/data/data_TMJA/tmja-departement-sum.csv", header=True)
data_TMJA_mean_dep.to_csv("Projet/data/data_TMJA/tmja-departement-mean.csv", header=True)
